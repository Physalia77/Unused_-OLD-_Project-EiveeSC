...
client = Sellix("F0bzz6kgOsRd2ZR3BH46wavmV6AZ7mbkXlVTyX8CPiSS4Nh85S37pPGJLmzpCJYG", "Eivee")
...
@bot.command(description=f"Help page for all related topics you could need help with around Eivee")  # Help command, command list
async def help(ctx):
    with open('prefixes.json', 'r') as f:
        prefixes = json.load(f)
    print(f'{datetime.now()}: {ctx.message.author} executed command - {prefixes[str(ctx.guild.id)]}{ctx.invoked_with}')
    ...
    embed = discord.Embed(title="Eivee's help command",
                          description=f" page for all related topics you could need help with around {bot.user.name}", color=0x3457db)
    for command in bot.walk_commands():
        description = command.description
        if not description or description is None or description == "":
            description = 'No description provided'
...
@bot.command(name="check_cogs", aliases=["check cog", "check", "c"], descreption="Look if cog/extension is loaded or not.")
...
async def load(ctx, extension=None):
    ...
    if extension is None:
        await ctx.send(embed=discord.Embed(title=f"Error: Arg extension missing [{getframeinfo(currentframe()).lineno}]", description="You have to name an extension that you want to enable", color=0xF30000, delete_after=7))
    else:
        try:
            ...
            await ctx.send(embed=discord.Embed(title=f"Error: `{getframeinfo(currentframe()).lineno}`",
                                          description=f'{format(extension)} cannot be loaded. [{format(error)}]', color=0xF30000, delete_after=7))
...
async def unload(ctx, extension=None):
    ...
    if extension is None:
        await ctx.send(embed=discord.Embed(title=f"Error: Arg extension missing [{getframeinfo(currentframe()).lineno}]", description="You have to name an extension that you want to disable", color=0xF30000, delete_after=7))
    else:
        try:
